...
void rpaddle() {
  ...
  if (MULTIPLAYER) {
    ...
    if (rpaddle_d == 1) {
      tft.fillRect(rpaddle_x, rpaddle_y, paddle_w, com_speed, BLACK);
    }
    else if (rpaddle_d == -1) {
      tft.fillRect(rpaddle_x, rpaddle_y + paddle_h - com_speed, paddle_w, com_speed, BLACK);
    }
    ...
    // Update Y Position
    rpaddle_y = rpaddle_y + rpaddle_d * com_speed;
...
void menu() {
  ...
  tft.drawString("6 points wins!", w / 2, h / 2 + 20, 4);
  delay(3000);
}
